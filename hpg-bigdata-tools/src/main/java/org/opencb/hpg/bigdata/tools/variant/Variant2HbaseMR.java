...
//          v.getEnd();
    ...
    public String buildStorageId(Variant v) {
        CharSequence chr = v.getReferenceName(); // TODO check for chr at chromosome name and remove it.
        ...
        if (v.getAlternateBases().size() > 1) {
            throw new NotImplementedException("More than one ALT per position not supported yet:" + builder.toString());
        }
