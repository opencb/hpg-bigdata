...
//		}
		throw new UnsupportedOperationException(
				String.format("Header line currently not supported: %s", hl.getClass()));
	}
	...
	private VCFInfoHeaderLine convertToInfo(VariantSetMetadata vsm){
		...
		VCFInfoHeaderLine hl = null;
		if(null != cl)
			hl = new VCFInfoHeaderLine(
					vsm.getId().toString(),
					cl,
					VCFHeaderLineType.valueOf(vsm.getType().toString()),
					vsm.getDescription().toString()
				);
		else 
			hl = new VCFInfoHeaderLine(
				vsm.getId().toString(),
				Integer.valueOf(vsm.getNumber().toString()),
				VCFHeaderLineType.valueOf(vsm.getType().toString()),
				vsm.getDescription().toString()
			);	
		return hl;
	...
	private VCFHeaderLine convertToFormat(VariantSetMetadata vsm) {
		String nbr = vsm.getNumber().toString();
		VCFHeaderLineCount cl = valueOfLineCountHeader(nbr);
		VCFFormatHeaderLine hl = null;
		if(null != cl)
			hl = new VCFFormatHeaderLine(
					vsm.getId().toString(),
					cl,
					VCFHeaderLineType.valueOf(vsm.getType().toString()),
					vsm.getDescription().toString()
	...
	public VCFHeaderLine backward(VariantSetMetadata vsm) {
		if(null == vsm){
			...
			throw new UnsupportedOperationException(String.format("Header line currently not supported: %s", vsm.getKey().toString()));
